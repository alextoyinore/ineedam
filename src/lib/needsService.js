...
export const fetchNeeds = async (from = 0, to = 9) => {
    const { data, error } = await supabase
        .from('needs')
        .select('*, profiles!needs_user_id_fkey(display_name, avatar_url, username, banner_url, bio, last_seen_at, kyc_status, location)')
        .neq('status', 'archived')
...
export const shapeNeed = (row) => {
    ...
    return {
        ...
        authorLocation: row.profiles?.location || null,
