 ...
 * Report a user.
 */
export const reportUser = async (reporterId: string, reportedId: string, reason: string, notes: string = '') => {
    const { data, error } = await supabase
        .from('user_reports')
        .insert([{
 ...
 */
export const checkBlockStatus = async (userId: string, otherUserId: string) => {
