 ...
 */
export const fetchUserThreads = async (userId: any) => {
    if (!userId) return [];
    ...
    return threadsData.map((thread: any) => {
        ...
        const otherParticipant = thread.thread_participants.find((tp: any) => tp.user_id !== userId);
        const myParticipant = thread.thread_participants.find((tp: any) => tp.user_id === userId);

        // Sort messages manually just in case, taking the last one
        const sortedMessages = thread.messages.sort((a: any, b: any) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
 ...
 */
export const getOrCreateThread = async (userId1: any, userId2: any) => {
    ...
    const p1Ids = p1?.map((p: any) => p.thread_id) || [];
    const p2Ids = p2?.map((p: any) => p.thread_id) || [];
 ...
 */
export const createMessage = async (threadId: any, senderId: any, text: any) => {
 ...
 */
export const markThreadAsReadInDb = async (threadId: any, userId: any) => {
